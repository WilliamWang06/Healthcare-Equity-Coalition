  ...
  const riCities = {
    ...
    columbus: {
      label: 'Columbus, OH',
      orgs: ['Shreyas Gorthy, Ohio State University', 'Sudhish Sakthivel, Olentangy Orange High School']
    },
    evanston: {
      label: 'Evanston, IL',
    ...
    cincinnati: {
      label: 'Cincinnati, OH',
      orgs: ['Shivam Vora, University of Cincinnati']
    }
  };
  ...
  if ('IntersectionObserver' in window) {
    ...
    reveals.forEach(el => {
      ...
      if (alreadyVisible) {
        // Already on screen at page load — show it immediately instead of
        // playing the ~1s fade/blur-in. Without this, anything above the
        // fold sits at opacity:0 for a moment right after load, which on a
        // slow connection (or a screenshot taken the instant the page
        // appears) reads as "the page is blank" even though it's about to
        // finish fading in on its own.
